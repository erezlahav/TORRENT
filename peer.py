...
import peer_signup_gui

TRACKER_IP = "10.68.121.52"
TRACKER_PORT = 5000
...
def update_progress_bar_loop(total_pieces):
    ...
    if not done:
        ...
        with CNT_DOWNLOADED_PIECES_LOCK:
            CNT_DOWNLOADED_PIECES = 0
            #reset after done for the next download
...
def Download_From_Peer(peer: dict):
    ...
    try:
        ...
        print("rsa public key gottttt")
        print(rsa_public_key_pem)
        # add checking rsa public key!!!

        if not is_valid_rsa_public_key(rsa_public_key_pem):
            mes = {"type" : "error","error number" : 1}
            send_with_size(download_sock,json.dumps(mes).encode())
            client_sock.close()
            return
        ...
        recvd_mes_dict = json.loads(recv_by_size(download_sock).decode())
        if recvd_mes_dict.get("type") != "SUCCESS":
            print("aes session key bad format sent")
            download_sock.close()
            return
        ...
        print("not able to connect to " + str(seeder_ip) + " , " + str(seeder_port))
    except TimeoutError:
...
def Download_Piece(download_sock: socket, info_hash: str, info_torrent_dict: dict, index: int,
...
def Handle_Downloading_Peer(download_sock: socket, read_sockets_list: list, rsa_keys: tuple):
    ...
    if dict_hello_mes == b'':
        ...
        #recv if success or not
        recvd_dict_mes = json.loads(recv_by_size(download_sock).decode())
        if recvd_dict_mes.get("type") != "SUCCESS":
            print("rsa public key bad format sent")
            read_sockets_list.remove(client_sock)
            client_sock.close()
            return
...
def Give_Pieces(sock: socket, info_hash, aes_session_key):
    ...
    while not stop:
        ...
        if recv_data != b'':
            ...
            P2P.Send_And_Encrypt_Give_Piece(sock, block_of_chunk, info_hash, chunk_index, block_size, current_offset,
                                            aes_session_key)
        else:
            stop = True
...
def Start_Seed_Thread():
    seed_thread = threading.Thread(target=Seed_Thread_Func, daemon=True)
    seed_thread.start()
...
def create_thread_status_update(sock: socket, peer_id):
    thread = threading.Thread(target=Send_Status_To_Tracker, args=(sock, peer_id), daemon=True)
    thread.start()
...
def ask_user_action() -> string:
    ...
    btn_download = tk.Button(root, text="Download", width=12, command=on_download)
    btn_download.pack(pady=5)

    btn_exit = tk.Button(root, text="Exit", width=12, command=on_exit)
...
def Sign_Up_To_Tracker():
    global PEER_ID
    global TRACKER_IP
    PORT = 5000
    client_sock = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
    try:
        client_sock.connect((TRACKER_IP, PORT))
        PEER_ID = peer_signup_gui.start_client(client_sock)
        return client_sock
    except Exception as e:
        print(e)
        sys.exit(0)

def Genrate_Seed_Port()-> int:
    while True:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            try:
                ...
                continue  # Port already in use, try another
...
if __name__ == "__main__":
    ...
    #Make_Torrent_File_For_New_File("messi.jpg")
    Verify_Chunks_In_Disk()
    load_all_torrents()

    client_sock = Sign_Up_To_Tracker()
    SEED_PORT = Genrate_Seed_Port()
    print(SEED_PORT)
    Start_Seed_Thread()
    bdata = recv_by_size(client_sock)
    print(bdata)
    create_thread_status_update(client_sock, PEER_ID)
    while True:  #main loop

        user_choice = ask_user_action()
        if user_choice == "exit":
            exit(0)
        with DONE_DOWNLOAD_LOCK:
            DONE_DOWNLOAD = False
